...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformedCaseStudies = caseStudies.map((caseStudy: CaseStudy) => {
      ...
      const transformedCaseStudy = formatItemUrls(
        caseStudy, 
        ['coverImageUrl', 'clientLogoUrl']
      );
      
      // Transform image URLs in actions
      if (transformedCaseStudy.actions && transformedCaseStudy.actions.length > 0) {
        transformedCaseStudy.actions = transformedCaseStudy.actions.map((action: Action) => 
          formatItemUrls(action, ['imageUrl'])
        );
      }
